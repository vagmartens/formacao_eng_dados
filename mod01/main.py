...
# retorno.text
retorno_text = retorno.text

df = pd.read_html(retorno_text)
print(type(df))

df = df[0].copy()
df

nr_populacao = list(range(1,26))
...
for index, row in df.iterrows():
    ...
    combinacao.append(str(v_pares)+ 'p-' + str(v_impares) + 'i-' + str(v_primos) + 'np' )
...
freq_nr.sort(key=lambda x: x[1])
freq_nr[0] # primeiro numero
freq_nr[-1] # ultimo numero

counter = collections.Counter(combinacao)
